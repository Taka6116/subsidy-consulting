      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
          {data.targetIndustries.map((industry, index) => (
            <div
              key={industry.label}
              className="rounded-xl bg-[#F3F6FA] p-5 text-center transition-shadow hover:shadow-md"
            >
              <p className="mb-3 text-xs font-bold text-[#008894]">{industry.label}</p>
              <div className="mb-3 flex aspect-square w-full items-center justify-center">
                <Image
              ...
              <p className="text-xs leading-relaxed text-gray-600">{industry.desc}</p>
            </div>
          ))}
        </div>
